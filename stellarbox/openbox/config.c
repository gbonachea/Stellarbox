...
static void parse_theme(xmlNodePtr node, gpointer d)
{
         ...
         n = obt_xml_find_node(n->next, "font"))
    {
        ...
        /* HiDPI: scale the font size so it stays legible at high dpi.  Only
           relative sizes (size >= 0, in points) are scaled; a negative size is
           an absolute pixel size and is left untouched. */
        size = (gint)((gdouble)size * RrThemeGetScale(ob_rr_inst) + 0.5);
