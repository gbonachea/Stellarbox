...
#ifdef SHAPE
/*! Apply a round-cornered mask to the frame window with the XShape
  extension.  The frame is cut so that the (decorated) top and bottom corners
  are curved with @a radius pixels.  The mask is removed for undecorated,
  maximized/fullscreen or client-shaped windows, which are kept rectangular.
  @param self The frame.
  @param radius Corner radius in pixels. */
static void frame_apply_rounded_corners(ObFrame *self, int radius)
{
    Display *d = obt_display;
    gint w = self->area.width;
    gint h = self->area.height;
    gboolean want_shaped;
    Pixmap mask;
    GC gc;
    XGCValues gcv;

    if (self->client->shaped)
        want_shaped = FALSE;    /* let the client's own shape win */
    else if (self->client->fullscreen ||
             (self->client->max_vert && self->client->max_horz))
        want_shaped = FALSE;    /* maximized/fullscreen fills the screen */
    else if (self->decorations &
             (OB_FRAME_DECOR_TITLEBAR | OB_FRAME_DECOR_BORDER |
              OB_FRAME_DECOR_HANDLE))
        want_shaped = TRUE;
    else
        want_shaped = FALSE;    /* utterly undecorated: stay rectangular */

    if (!want_shaped) {
        /* undo any rounding, back to the plain frame window */
        XShapeCombineMask(d, self->window, ShapeBounding, 0, 0, None, ShapeSet);
        return;
    }
    ...
    /* 1-bit mask: 0 = transparent (cut away), 1 = visible */
    mask = XCreatePixmap(d, self->window, w, h, 1);
    gc = XCreateGC(d, mask, 0, &gcv);

    /* clear the whole mask first */
    XSetForeground(d, gc, 0);
    XFillRectangle(d, mask, gc, 0, 0, w, h);

    /* the visible shape: four straight rails plus four corner discs */
    XSetForeground(d, gc, 1);
    XFillRectangle(d, mask, gc, radius, 0, w - radius * 2, h);
    XFillRectangle(d, mask, gc, 0, radius, w, h - radius * 2);
    XFillArc(d, mask, gc, 0, 0, radius * 2, radius * 2, 0, 360 * 64);
    XFillArc(d, mask, gc, w - radius * 2, 0, radius * 2, radius * 2,
             0, 360 * 64);
    XFillArc(d, mask, gc, 0, h - radius * 2, radius * 2, radius * 2,
             0, 360 * 64);
    XFillArc(d, mask, gc, w - radius * 2, h - radius * 2, radius * 2,
             radius * 2, 0, 360 * 64);

    XShapeCombineMask(d, self->window, ShapeBounding, 0, 0, mask, ShapeSet);
...
#ifdef SHAPE
            /* round the corners; the radius tracks the pixel density */
            frame_apply_rounded_corners(
                self, MAX(1, (gint)(8.0 * RrThemeGetScale(ob_rr_inst))));
#endif
        }
...
void frame_flash_start(ObFrame *self)
{
    ...
    if (!self->flashing)
        self->flash_timer = g_timeout_add_full(G_PRIORITY_DEFAULT,
                                               ...
                                               flash_done);
    self->flash_end = g_get_monotonic_time() + G_USEC_PER_SEC * 5;
...
static gulong frame_animate_iconify_time_left(ObFrame *self,
                                              gint64 now)
{
    gint64 time = self->iconify_animation_end - now;
    /* no negative values */
    return MAX(time, 0);
}
...
void frame_begin_iconify_animation(ObFrame *self, gboolean iconifying)
{
    ...
    /* set the ending time */
    if (set_end)
        self->iconify_animation_end = now + time;
