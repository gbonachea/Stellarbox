...
void ob_debug_startup(void)
{
    ...
    if (!obt_paths_mkdir_path(dir, 0777))
        ...
        gchar *name = g_build_filename(obt_paths_cache_home(p),
                                       "stellarbox", "stellarbox.log", NULL);
    ...
    obt_handler_id =
        g_log_set_handler("Obt", G_LOG_LEVEL_MASK | G_LOG_FLAG_FATAL |
                          G_LOG_FLAG_RECURSION, log_handler, NULL);
    ob_handler_id =
        g_log_set_handler("Stellarbox", G_LOG_LEVEL_MASK | G_LOG_FLAG_FATAL |
                          G_LOG_FLAG_RECURSION, log_handler, NULL);
    ob_handler_prompt_id =
        g_log_set_handler("Stellarbox", G_LOG_LEVEL_MASK & ~G_LOG_LEVEL_DEBUG,
                          prompt_handler, NULL);
...
void ob_debug_shutdown(void)
{
    ...
    g_log_remove_handler("Stellarbox", ob_handler_id);
    g_log_remove_handler("Stellarbox", ob_handler_prompt_id);
...
void ob_debug_show_prompts(void)
{
    if (prompt_queue) {
        ...
        while (prompt_queue) {
            prompt_show_message(prompt_queue->data, "Stellarbox", _("Close"));
