...
gboolean screen_annex(void)
{
    ...
    OBT_PROP_SETS(screen_support_win, NET_WM_NAME, "Stellarbox");
    ...
    supported[i++] = OBT_PROP_ATOM(NET_WM_OPAQUE_REGION);
                       ...
                       NET_NUMBER_OF_DESKTOPS, CARDINAL, &d))
    {
        if (d != config_desktops_num) {
            /* TRANSLATORS: If you need to specify a different order of the
               ...
               "The current session has %2$d desktops, but Stellarbox is configured for %1$d ..." */
            g_warning(ngettext("Stellarbox is configured for %d desktop, but the current session has %d.  Overriding the Stellarbox configuration.", "Stellarbox is configured for %d desktops, but the current session has %d.  Overriding the Stellarbox configuration.", config_desktops_num),
