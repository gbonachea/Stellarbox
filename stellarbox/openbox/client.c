...
void client_manage(Window window, ObPrompt *prompt)
{
    ...
    */
    ...
    /* tell the compositor where our frame is truly opaque */
    client_update_opaque_region(self);
...
void client_update_opacity(ObClient *self)
{
    ...
    /* EWMH: 0xffffffff means fully opaque, i.e. the same as not having the
       property at all, so only mirror values that actually make the window
       translucent */
...
/*! Update the _NET_WM_OPAQUE_REGION hint on the frame window.
  The hints are expressed in frame-window coordinates and describe the region
  of the frame that is guaranteed to be fully opaque - the client area.  The
  rest of the frame (the possibly curved / cut-out border, the titlebar and
  the frame decorations) is left outside of the region, so the compositor
  knows it may be transparent and draws the drop shadow around the actual
  frame outline instead of underneath the transparent corners. */
void client_update_opaque_region(ObClient *self)
{
    gulong r[4];

    r[0] = self->frame->size.left;
    r[1] = self->frame->size.top;
    r[2] = self->area.width;
    r[3] = self->area.height;

    OBT_PROP_SETA32(self->frame->window, NET_WM_OPAQUE_REGION,
                    CARDINAL, r, 4);
}
                      ...
                      gboolean user, gboolean final, gboolean force_reply)
{
    ...
    */
    ...
    /* keep the compositor's opaque-region hint in sync with the geometry */
    if (fmoved || fresized)
        client_update_opaque_region(self);
}
