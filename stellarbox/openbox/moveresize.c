...
/* ---- edge snapping (drag a window against a screen edge/corner) ---- */
/* how close (px) the pointer must be to a monitor edge to trigger a snap */
#define SNAP_EDGE_TOLERANCE 4
/* alpha of the Cairo preview rectangle (only with a 32-bit visual) */
#define SNAP_PREVIEW_ALPHA 0.35
...
/* edge snapping state */
static gboolean snap_enabled = FALSE; /* this window may be snapped */
static gboolean snap_active = FALSE;  /* the preview is currently shown */
static Rect snap_target;              /* the target geometry (if any) */
static Rect snap_area;                /* work area of the snapping monitor */
static Window snap_preview_win = None;
...
static void snap_preview_free(void);
static gboolean snap_detect(gint x, gint y, Rect *out);
#ifdef SYNC
 ...
 * Edge snapping
 *
 * While a window is being dragged and the pointer reaches the edge (or a
 * corner) of the monitor, the fork proposes a "snap" target - half of the
 * work area for a plain edge, a quarter for a corner.  A translucent Cairo
 * rectangle paints the preview over the screen, and the target geometry is
 * applied when the button is released (see moveresize_end()).
 * ------------------------------------------------------------------------- */

static void snap_preview_create(void)
{
    XSetWindowAttributes attrib;

    if (snap_preview_win != None)
        return;
    ...
    snap_preview_win = XCreateWindow(obt_display, obt_root(ob_screen),
                                     0, 0, 1, 1, 0,
                                     RrDepth(ob_rr_inst),
                                     ...
                                     &attrib);
    XStoreName(obt_display, snap_preview_win, "snap-preview");
}

static void snap_preview_show(const Rect *a)
{
    cairo_surface_t *s;
    cairo_t *cr;
    const gdouble alpha = (RrDepth(ob_rr_inst) == 32) ?
        SNAP_PREVIEW_ALPHA : 1.0;

    snap_preview_create();

    XMoveResizeWindow(obt_display, snap_preview_win,
                      a->x, a->y, a->width, a->height);

    /* raise above the frames; re-raising each motion keeps it on top while
       the pointer grazes the screens edge */
    XRaiseWindow(obt_display, snap_preview_win);
    XMapWindow(obt_display, snap_preview_win);

    s = cairo_xlib_surface_create(obt_display, snap_preview_win,
                                  RrVisual(ob_rr_inst), a->width, a->height);
    cr = cairo_create(s);

    /* translucent blue fill */
    cairo_set_source_rgba(cr, 0.18, 0.47, 0.93, alpha);
    cairo_paint(cr);

    /* an inner outline, so the target zone reads clearly */
    cairo_set_source_rgba(cr, 1.0, 1.0, 1.0,
                          RrDepth(ob_rr_inst) == 32 ? 0.55 : 1.0);
    cairo_set_line_width(cr, 2.0);
    cairo_rectangle(cr, 1.5, 1.5, a->width - 3, a->height - 3);
    cairo_stroke(cr);
...
static void snap_preview_hide(void)
{
    if (snap_preview_win == None)
        return;
    XUnmapWindow(obt_display, snap_preview_win);
    XFlush(obt_display);
}
...
{
    if (snap_preview_win != None) {
        XDestroyWindow(obt_display, snap_preview_win);
        snap_preview_win = None;
    }
}

/*! Compute the snap target for the pointer at (x, y).
  @return TRUE and fill @a out when the pointer is within snapping distance
  of the edge/corner of the monitor that contains it. */
static gboolean snap_detect(gint x, gint y, Rect *out)
{
    const Rect *phys;
    Rect *area;
    ...
    head = screen_monitor_pointer();
    area = screen_area(screen_desktop, head, NULL);
    snap_area = *area;
    g_slice_free(Rect, area);

    phys = screen_physical_area_monitor(head);

    /* near the edges of the *physical* monitor (struts may shift the work
       area itself) */
    left   = (x <= RECT_LEFT(*phys)   + SNAP_EDGE_TOLERANCE);
    right  = (x >= RECT_RIGHT(*phys)  - SNAP_EDGE_TOLERANCE);
    top    = (y <= RECT_TOP(*phys)    + SNAP_EDGE_TOLERANCE);
    bottom = (y >= RECT_BOTTOM(*phys) - SNAP_EDGE_TOLERANCE);

    if (!(left || right || top || bottom))
        return FALSE;

    /* start from the full work area */
    RECT_SET(*out, snap_area.x, snap_area.y,
             snap_area.width, snap_area.height);

    if ((left || right) && (top || bottom)) {
        /* corner: a 25% quarter of the work area */
        hw = snap_area.width / 2;
        hh = snap_area.height / 2;
        out->width = hw;
        out->height = hh;
        if (right) out->x += hw;
        if (bottom) out->y += hh;
        ...
        /* edge: a 50% half, full height */
        hw = snap_area.width / 2;
        out->width = hw;
        if (right) out->x += hw;
    }
    else {
        /* top or bottom edge: full width, 50% height */
        hh = snap_area.height / 2;
        out->height = hh;
        if (bottom) out->y += hh;
    }
...
void moveresize_start(ObClient *c, gint x, gint y, guint b, guint32 cnr)
{
    ...
    */
    ...
    /* edge snapping only makes sense when dragging a resizable window that
       is not already filling the screen */
    snap_active = FALSE;
    snap_enabled = moving &&
        (c->functions & OB_CLIENT_FUNC_RESIZE) &&
...
void moveresize_end(gboolean cancel)
{
    ...
    if (!cancel && snap_active) {
        /* the pointer reached an edge/corner: apply the snapped geometry
           instead of the raw drag position */
        gboolean hmax = moveresize_client->max_horz;
        gboolean vmax = moveresize_client->max_vert;

        if (hmax || vmax) {
            /* remember to put the halves/quarters back on cancel */
            was_max_horz = hmax;
            was_max_vert = vmax;
            if (hmax) {
            ...
            if (hmax) client_maximize(moveresize_client, FALSE, 1);
            if (vmax) client_maximize(moveresize_client, FALSE, 2);
        }

        cur_x = snap_target.x;
        cur_y = snap_target.y;
        cur_w = snap_target.width;
        cur_h = snap_target.height;
    }
...
gboolean moveresize_event(XEvent *e)
{
    ...
    if (e->type == ButtonPress) {
        ...
        if (moving) {
            ...
            /* edge snapping: paint/refresh the preview while the pointer is
               against an edge/corner; the geometry is applied on release */
            if (snap_enabled &&
                snap_detect(e->xmotion.x_root, e->xmotion.y_root,
                            &snap_target))
            {
                snap_active = TRUE;
                snap_preview_show(&snap_target);
            }
            else if (snap_active) {
                snap_active = FALSE;
                snap_preview_hide();
