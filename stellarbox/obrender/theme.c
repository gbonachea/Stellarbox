 ...
 * HiDPI support
 *
 * Openbox was written for a single logical pixel density (usually 96 dpi).  On
 * high resolution displays every fixed geometry (title bar height, border
 * width, button size, ...) and every font comes out looking tiny, so the fork
 * scales everything by a pixel-density factor: dpi / 96.  The factor is
 * detected once per process and cached.
 * ------------------------------------------------------------------------- */

static double theme_scale_factor = 0.0; /* 0.0 == not detected yet */

/*! Compute the pixel-density scale on this display.
  @param inst The rendering instance. */
static double theme_detect_scale(const RrInstance *inst)
{
    const char *v;
    double dpi = 0.0;

    /* first ask Xft (XSETTINGS/xresources), the value every toolkit trusts */
    v = XGetDefault(RrDisplay(inst), "Xft", "dpi");
    if (v) {
        dpi = g_ascii_strtod(v, NULL);
        if (dpi >= 24.0 && dpi <= 1000.0)
            return dpi / 96.0;
    }

    /* fall back to the physical pixel density of the default screen */
    {
        gint wmm = DisplayWidthMM(RrDisplay(inst), RrScreen(inst));
        gint wpix = DisplayWidth(RrDisplay(inst), RrScreen(inst));

        if (wmm > 0 && wpix > 0)
            dpi = (gdouble)wpix * 254.0 / (gdouble)wmm / 100.0;
    }
    if (dpi >= 24.0 && dpi <= 1000.0)
        return dpi / 96.0;
...
double RrThemeGetScale(const RrInstance *inst)
{
    if (theme_scale_factor <= 0.0)
        theme_scale_factor = theme_detect_scale(inst);
    return theme_scale_factor;
}

void RrThemeSetScale(double scale)
{
    theme_scale_factor = scale > 0.0 ? scale : 1.0;
}

/* ---------------------------------------------------------------------------
 * Cairo rendering support
 *
 * The classic Openbox renderer paints XPM/XBM pixmaps into an offscreen Pixmap
 * and glues it to a window with XSetWindowBackgroundPixmap().  On top of that
 * (and without touching it) this fork draws vector geometry with Cairo: the
 * surface below is bound directly to an X11 Drawable - the frame container
 * window or any of its children - so anti-aliased arcs, strokes and PangoCairo
 * text can be layered over the themed background.
 * ------------------------------------------------------------------------- */

cairo_surface_t *RrThemeCairoSurface(const RrTheme *theme, Drawable drawable,
                                     gint w, gint h)
{
    ...
    s = cairo_xlib_surface_create(RrDisplay(theme->inst), drawable,
                                  RrVisual(theme->inst), w, h);
    g_assert(cairo_surface_status(s) == CAIRO_STATUS_SUCCESS);
    return s;
}

cairo_t *RrThemeCairoContext(const RrTheme *theme, Drawable drawable,
                             gint w, gint h, cairo_surface_t **surface)
{
    cairo_surface_t *s;
    cairo_t *cr;

    s = RrThemeCairoSurface(theme, drawable, w, h);
    if (s == NULL)
        return NULL;
    if (surface)
        *surface = s;
...
void RrThemeCairoFinish(const RrTheme *theme, cairo_t *cr,
                    ...
                    RrFont *active_osd_font, RrFont *inactive_osd_font)
{
    ...
    /* load direct dimensions (scaled by the pixel density: HiDPI) */
    {
        gdouble sc = RrThemeGetScale(inst);
        gint scaled_overlap;

        READ_INT("menu.overlap", menu_overlap, -100, 100, 0);
        scaled_overlap = (gint)((gdouble)menu_overlap * sc + 0.5);
        READ_INT("menu.overlap.x", theme->menu_overlap_x,
                 -100, 100, scaled_overlap);
        READ_INT("menu.overlap.y", theme->menu_overlap_y,
                 -100, 100, scaled_overlap);
        READ_INT("window.handle.width", theme->handle_height,
                 0, 100, (gint)(6 * sc + 0.5));
        READ_INT("padding.width", theme->paddingx,
                 0, 100, (gint)(3 * sc + 0.5));
        READ_INT("padding.height", theme->paddingy,
                 0, 100, (gint)((gdouble)theme->paddingx * sc + 0.5));
        READ_INT("border.width", theme->fbwidth,
                 0, 100, (gint)(1 * sc + 0.5));
        READ_INT("menu.border.width", theme->mbwidth,
                 0, 100, theme->fbwidth);
        READ_INT("osd.border.width", theme->obwidth,
                 0, 100, theme->fbwidth);
        READ_INT("undecorated.border.width", theme->ubwidth,
                 0, 100, theme->fbwidth);
        READ_INT("menu.separator.width", theme->menu_sep_width,
                 1, 100, (gint)(1 * sc + 0.5));
        ...
        READ_INT("menu.separator.padding.height", theme->menu_sep_paddingy,
                 0, 100, (gint)(3 * sc + 0.5));
        READ_INT("window.client.padding.width", theme->cbwidthx,
                 0, 100, theme->paddingx);
        READ_INT("window.client.padding.height", theme->cbwidthy,
                 0, 100, theme->cbwidthx);
    }
    ...
    theme->button_size = MAX(theme->label_height - 2,
                             (gint)(RrThemeGetScale(inst) * 18.0 + 0.5));
    /* keep the titlebar tall enough for the (possibly larger) buttons */
    theme->title_height = MAX(theme->title_height,
                              theme->button_size + theme->paddingy * 2 +
                              (gint)(RrThemeGetScale(inst) * 2.0 + 0.5));
    theme->grip_width = (gint)(25 * RrThemeGetScale(inst) + 0.5);
    theme->scale_factor = RrThemeGetScale(inst);
...
static XrmDatabase loaddb(const gchar *name, gchar **path)
{
    ...
    if (name[0] == '/') {
        ...
        {
            /* switched to a Stellarbox-dedicated theme tree so that the
               package can coexist with a system Openbox instalation */
            s = g_build_filename(it->data, "stellarbox", "themes", name,
                                 "openbox-3", "themerc", NULL);
            if ((db = XrmGetFileDatabase(s)))
                *path = g_path_get_dirname(s);
            g_free(s);
