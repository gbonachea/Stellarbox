...
struct _RrTheme {
    ...
    /* pixel-density scale (HiDPI): detected factor or one forced by the
       configuration, see RrThemeGetScale()/RrThemeSetScale() */
...
/*! Pixel-density scale (dpi/96), detected once per process. */
double RrThemeGetScale(const RrInstance *inst);
void RrThemeSetScale(double scale);

/*! Cairo helpers for vector rendering bound to an X11 drawable. */
