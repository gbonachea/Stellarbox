...
RrInstance* RrInstanceNew (Display *display, gint screen)
{
    ...
    /* a PangoCairo context: all text is rendered with PangoCairo (see
       RrFontDrawCairo), so the layouts must come from the cairo font map */
    {
        PangoFontMap *fmap = pango_cairo_font_map_get_default();
        definst->pango = pango_font_map_create_context(fmap);
        g_object_unref(fmap);
    }
