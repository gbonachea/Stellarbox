...
Pixmap RrPaintPixmap(RrAppearance *a, gint w, gint h)
{
    ...
    for (i = 0; i < a->textures; i++) {
        ...
        case RR_TEXTURE_TEXT:
            ...
            {
                /* render the text with PangoCairo into a surface bound to
                   the pixmap */
                cairo_surface_t *s;
                cairo_t *cr;

                s = cairo_xlib_surface_create(RrDisplay(a->inst), a->pixmap,
                                              RrVisual(a->inst), w, h);
                g_assert(cairo_surface_status(s) == CAIRO_STATUS_SUCCESS);
                cr = cairo_create(s);
                RrFontDrawCairo(cr, &a->texture[i].data.text, &tarea);
                cairo_destroy(cr);
