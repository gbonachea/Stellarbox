...
/*! Shared layout setup for RrFontDraw() and RrFontDrawCairo(): the text that
  the texture points at is loaded into the pango layout and the target
  position/size is computed. */
static void font_prepare_layout(RrTextureText *t, RrRect *area,
                                gint *x, gint *y, gint *w, gint *mw)
{
    PangoRectangle rect;
    ...
    *mw = rect.width;
    ...
    /* pango_layout_set_alignment doesn't work with the per-line renderers */
    switch (t->justify) {
...
/*! PangoCairo version of RrFontDraw(). Draws the text texture into a Cairo
  context that is bound to an X11 drawable (see RrThemeCairoContext()). */
void RrFontDrawCairo(cairo_t *cr, RrTextureText *t, RrRect *area)
{
    gint x, y, w, mw;
    PangoAttrList *attrlist;

    font_prepare_layout(t, area, &x, &y, &w, &mw);

    if (t->shadow_offset_x || t->shadow_offset_y) {
        cairo_save(cr);
        cairo_set_source_rgba(cr,
                              t->shadow_color->r / 255.0,
                              t->shadow_color->g / 255.0,
                              t->shadow_color->b / 255.0,
                              t->shadow_alpha / 255.0);
        cairo_move_to(cr, x + t->shadow_offset_x,
                      y + t->shadow_offset_y);
        if (!t->flow) {
    ...
    (PANGO_VERSION_MAJOR == 1 && PANGO_VERSION_MINOR >= 16)
            ...
            pango_cairo_show_layout(cr, t->font->layout);
        }
        cairo_restore(cr);
    }
